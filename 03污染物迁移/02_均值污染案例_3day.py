...
OUTPUT_DIR = Path(r"D:\1document\Graduation Thesis\01Code\DATA\PM2.5_Meteorology_Study\PM2.5_Meteorology_Study_3day")
...
def find_refined_golden_events(df):
    ...
    for year in range(2020, 2024):
        ...
        # 【微调后的窗口】：更加聚焦迁移过程
        # 从爆发前 24 小时开始（1天背景）
        # 到峰值后 48 小时结束（2天演变与消散）
        start_show = burst_time - pd.Timedelta(days=1)
        end_show = peak_time + pd.Timedelta(days=2)
...
def main():
    ...
    plt.title("杭州市PM2.5四年冬季爆发时段", fontsize=16)
    ...
    plt.savefig(OUTPUT_DIR / "Hangzhou_Timeline_Calibration.png", dpi=300)
