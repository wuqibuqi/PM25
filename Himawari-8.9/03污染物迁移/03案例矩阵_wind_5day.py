...
CASES_CSV = Path(r"D:\1document\Graduation Thesis\01Code\DATA\PM2.5_Meteorology_Study\PM2.5_Meteorology_Study_5day\Refined_Golden_Cases_Selection.csv")
OUT_DIR = Path(r"D:\1document\Graduation Thesis\01Code\DATA\PM2.5_Meteorology_Study\PM2.5_Meteorology_Study_5day\Evolution_Real_Wind")
...
def read_raster_and_coords(tif_path):
    ...
    with rasterio.open(tif_path) as src:
        ...
        data[np.abs(data) > 100] = np.nan  # 风速通常不会超过 100m/s，剔除异常值

        bounds = src.bounds
        # 关键：lon 从西向东，lat 从南向北
        lons = np.linspace(bounds.left, bounds.right, src.width)
        lats = np.linspace(bounds.bottom, bounds.top, src.height)

        # 核心：因为 src.read(1) 返回的矩阵第一行是北边（最高纬度）
        # 而我们的 lats 是从南向北生成的，所以需要将数据矩阵进行上下翻转
        data = np.flipud(data)
