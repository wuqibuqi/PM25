...
def run_pscf_multi_height():
    ...
    for idx, row in df_cases.iterrows():
        ...
        plt.title(f"杭州冬季 $PM_{{2.5}}$ 综合潜在源区解析 ({case_tag})\n" +
                  f"基于 100/500/1000m 融合轨迹 | 峰值浓度: {hz_pm:.1f} $\mu g/m^3$", fontsize=14, fontweight='bold',
                  pad=15)

        cbar = plt.colorbar(mesh, fraction=0.03, pad=0.04)
        cbar.set_label('WPSCF 综合贡献强度', fontsize=12)

        plt.savefig(SAVE_DIR / f"WPSCF_Case_{idx}_MultiHeight.png", dpi=300, bbox_inches='tight')
        plt.close()


if __name__ == "__main__":
    run_pscf_multi_height()